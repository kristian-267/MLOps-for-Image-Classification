...
def train(config):
    ...
    Path(paths.profile_path + config.experiment.name).mkdir(parents=True, exist_ok=True)

    wandb_logger = WandbLogger(name=config.experiment.name, save_dir=paths.log_path + config.experiment.name, project=config.wandb.project, log_model=config.wandb.log_model)
    wandb_logger.experiment.config.update(OmegaConf.to_container(config.experiment, resolve=True, throw_on_missing=True))
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(dirpath=paths.model_path + hparams.name, monitor=hparams.monitor, mode=hparams.monitor_mode, save_on_train_epoch_end=True, every_n_train_steps=1)
    early_stopping_callback = pl.callbacks.EarlyStopping(monitor=hparams.monitor, patience=hparams.es_patience, verbose=True, mode=hparams.monitor_mode)

...
>
> Answer:
We have organized our CI into 5 separate files: Docker Image CI, Run black, Run flake8, Run isort, and Run tests. While black, flake8 and isort functions to check the formatting of codes and order files alphabetically, Docker Image CI builds a new docker image into the dockerhub everytime when there's an update, and the Run tests workflow functions to trigger unittesting from pytest and report a coverage rate.
